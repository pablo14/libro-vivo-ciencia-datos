...
* `r wemoji("bar_chart")` Sólo con datos categóricos
* `r wemoji("straight_ruler")` Sólo con datos numéricos
* `r wemoji("bar_chart")``r wemoji("straight_ruler")` Con ambos tipos
...
Es una buena práctica asignar el valor mínimo y máximo usando una función. En este caso, la variable es un porcentaje, por lo que sabemos de antemano que su escala es de 0 a 100; sin embargo, `r wemoji("warning")` _¿qué pasaría si no conociéramos el rango?_ 
...
**¡Todos los segmentos cambiaron!** `r wemoji("scream")` Dado que estas son nuevas categorías, el modelo predictivo fallará a la hora de procesarlas porque son todos valores nuevos.
...
#### Bonus track: El arte del equilibrio `r wemoji("balance_scale")`
...
### El caso `r wemoji("mag")` {#analizar-resultados-binarios-vs-variable-categorica}
...
### Análisis para el modelado predictivo `r wemoji("crystal_ball")`
...
**Mejora: ~ `r round(100*(new_roc-roc)/roc,2)`%**. `r wemoji("white_check_mark")`
...
### Variable objetivo numérica o multinomial `r wemoji("straight_ruler")`
...
### ¿Qué beneficio "extra" `r wemoji("gift")` obtuvimos con la agrupación?
...
(`r wemoji("sunglasses")`). Entonces, surge la pregunta: _¿Dónde definimos el umbral de lo extremo?_ Basándonos en la intuición, puede ser el 1% más alto, o podemos analizar cómo cambia el promedio si quitamos el 1% más alto.
...
`r wemoji("snowman")` `r wemoji("beach_umbrella")`

`r wemoji("japanese_goblin")`: _"¡Oh! ¡Pero eso sería tomar un ejemplo extremo con dos locaciones diferentes!"_
...
`r wemoji("thinking")`: _"Ok. Pero, ¿cómo se relacionan estos conceptos con machine learning?"_
...
`r wemoji("pushpin")` Volviendo al problema original (_¿Dónde termina el clima frío?_), no todas las preguntas necesitan una respuesta: algunas solamente nos ayudan a pensar.
...
#### Comunicar los resultados `r wemoji("earth_africa")` `r wemoji("mega")`
...
* **Numéricos** `r wemoji("straight_ruler")`: como los que vimos antes:
...
¡Oh `r wemoji("scream")`! La combinación de `Argentina` y `cat_B` es _realmente baja_ (0.75%) en comparación con los otros valores (menos del 1%), mientras que las otras intersecciones están por encima del 22%.
...
### Paso 1: Cómo detectar valores atípicos `r wemoji("mag_right")`
...
### Paso 2: ¿Qué hacemos con los valores atípicos? `r wemoji("hammer_and_wrench")`

Ya detectamos qué puntos son los atípicos. Ahora, la pregunta es _¿Qué hacemos con ellos?_ `r wemoji("thinking")`
...
_Esta última tarea de mirar algunos números y visualizar la distribución de la variable es como imaginar una fotografía por lo que otra persona nos dice: convertimos la voz (que es una señal) en una imagen en nuestro cerebro._ `r wemoji("speaking_head")` `r wemoji("roll_eyes")` ... => `r wemoji("mountain_snow")` 
...
Ejemplificaremos estos temas con un enfoque práctico en R. Este código busca ser lo suficientemente genérico para que lo puedan aplicar en sus proyectos `r wemoji("slightly_smiling_face")`.
...
Aquí aparece una pregunta importante: _¿Qué hacemos?_ `r wemoji("scream")`
...
Como pueden ver, existe una interrelación entre todos los conceptos. `r wemoji("wink")`.
...
**Consejo:** Como primer abordaje en la imputación de los valores faltantes, este método es sumamente complejo. `r wemoji("fearful")`. 
...
_¡Seguimos sin llegar a cero! ¡¿Cuánto tiempo necesitamos?!_ `r wemoji("scream")`
...
Pero esto también ocurre en _"la vida real"_, como cuando escribimos un libro de análisis de datos para todas las edades, ¿cuánto tiempo necesitamos para terminarlo? ¿Una cantidad infinita? Probablemente no. `r wemoji("smile")`.
